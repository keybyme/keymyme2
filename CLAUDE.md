...
- **`accounts`** — `CustomUser` (`AUTH_USER_MODEL = "accounts.CustomUser"`), extending `AbstractUser` with `is_admin_principal`, `storage_quota_gb`/`storage_used_bytes`, `is_suspended`, `created_by`. Storage quota checks go through `user.has_space_for(bytes)` / `storage_available_bytes`, not ad-hoc math. Permission checks go through `user.has_permission(submodule_codename)`, which checks `UserPermissionOverride` first (per-user grant/revoke), then falls back to the user's currently active `Role`s (see `menus.UserRole` below). `user.role_level` returns the highest `level` among those active roles (0 if none), and `user.active_roles` returns the `Role` queryset itself.
- **`menus`** — dynamic permission/menu system: `Module` → `SubModule` (e.g. codename `docs.upload`) → `Role` (reusable permission sets, via `RolePermission`) → optional per-user `UserPermissionOverride`. This is the source of truth for what a user can see/do; it's independent of Django's built-in `auth.Permission`. A user can hold multiple `Role`s at once via `UserRole` (`user`, `role`, `valid_from`, `valid_until`) — the admin assigns/removes these through a `CustomUser` inline in `/admin`. Leaving both dates blank makes the assignment permanent; setting either scopes it to a date range (inclusive), letting the admin grant a role temporarily. `CustomUser._active_user_roles()` (`accounts/models.py`) is what filters to only the `UserRole` rows currently in effect (today's date per `timezone.localdate()`, i.e. `America/New_York`) — both `has_permission` and `role_level` go through it, so expired/not-yet-started role grants are automatically excluded everywhere.
